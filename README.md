...
> ⚠️ **Important Note**: The test cases in this repository  sourced from the **MOSIP Inji Collaboration Guide** are referenced using **inji-number** identifiers. Previously, the valid test cases worked correctly on the official Inji Verify website https://injiverify.collab.mosip.net/scan. However, they may no longer function properly on the official site as well as ours, possibly due to recent changes in the DID document or public key by your side. Please verify the current status of these test cases against the latest MOSIP documentation before using them for testing in our website.While shooting the video they were working in the official inji verify website as well as our website.The invalid and the expired are still working.
