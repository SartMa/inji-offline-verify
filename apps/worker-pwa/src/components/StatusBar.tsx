...
const StatusBar: React.FC = () => {
  ...
  const statusItems = [
    ...
    },
    {
      ...
      description:
        stats.pendingSyncCount > 0 ? `${stats.pendingSyncCount} items waiting to sync` : 'All items synced',
  ...
  return (
    <div className="status-bar-container">
      <div className="status-bar-header">
        ...
        <div className="status-timestamp">Last updated: {new Date().toLocaleTimeString()}</div>
      </div>

      <div className="status-grid">
        {statusItems.map((item, index) => (
          <div key={item.label} className={`status-card status-${item.status}`} style={{ animationDelay: `${index * 0.1}s` }}>
            ...
            <div className="status-value-container">
              <div className={`status-value status-value-${item.status}`}>{item.value}</div>
