...
const TestInterface: React.FC = () => {
  ...
  const simulateBlackBoxOutput = async () => {
    ...
    try {
      ...
      const result = await storeVerificationResult(data as any);
      if (result) {
        setLastResult({
          success: true,
  ...
  const generateSampleJSON = (type: 'success' | 'failure' | 'random' = 'random') => {
    let sample: any;
  ...
  return (
    <div className="test-interface-container">
      <div className="test-header">
        ...
        <div className="test-subtitle">Simulate verification output data</div>
      </div>

      <div className="test-description">
        <p>
          This interface simulates the output from your verification black box. Paste or generate JSON
          data to test the storage and sync functionality.
        </p>
      </div>
      ...
      <div className="templates-section">
        ...
        <div className="templates-grid">
          {presetTemplates.map((template) => (
            <button key={template.type} onClick={() => generateSampleJSON(template.type)} className="template-button">
      ...
      <div className="input-section">
        <div className="input-header">
          ...
          <div className="input-controls">
            <button onClick={formatJSON} disabled={!jsonInput.trim() || !isValid} className="control-btn format-btn" title="Format JSON">
              📐 Format
            </button>
            <button onClick={clearInput} disabled={!jsonInput.trim()} className="control-btn clear-btn" title="Clear Input">
              🗑️ Clear
            </button>
          </div>
        </div>
        ...
        <div className={`json-input-container ${!isValid ? 'invalid' : ''}`}>
          ...
          />
          {!isValid && <div className="validation-error">⚠️ Invalid JSON format</div>}
        </div>
      ...
      <div className="action-section">
        <button onClick={simulateBlackBoxOutput} disabled={!jsonInput.trim() || !isValid || isLoading} className="store-button">
          {isLoading ? (
            <>
      ...
      {lastResult && (
        <div className={`result-section result-${lastResult.success ? 'success' : 'error'}`}>
          <div className="result-header">
            <span className="result-icon">{lastResult.success ? '✅' : '❌'}</span>
      ...
      <div className="help-section">
        <details className="help-details">
          ...
          <div className="help-content">
            ...
            <ul>
              <li>
                <code>status</code> - "success" or "failure"
              </li>
              <li>
                <code>issuer</code> - Name of the credential issuer
              </li>
              <li>
                <code>subject</code> - Subject of the credential
              </li>
            </ul>
            <h4>Optional Fields:</h4>
            <ul>
              <li>
                <code>vcData</code> - Verifiable credential data
              </li>
              <li>
                <code>verificationDetails</code> - Verification results
              </li>
              <li>
                <code>error</code> - Error message (for failures)
              </li>
            </ul>
          </div>
        </details>
