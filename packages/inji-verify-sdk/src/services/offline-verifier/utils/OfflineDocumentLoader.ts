...
//     const db = await openDB(); // open DB before any use

//     // 1) DID resolution via key cache
...
import { openCacheDB, getContext, getKeyById, getAnyKeyForDid } from '../cache/utils/CacheHelper';

/**
 * Resolves DID documents and JSON-LD contexts from SDK-managed cache.
 * If online and a context is missing, it is fetched once and cached.
 */
function vmFromKey(key: any) {
  return {
    '@context': ['https://w3id.org/security/suites/ed25519-2020/v1'],
...
export class OfflineDocumentLoader {
  ...
  async documentLoader(url: string) {
    ...
    // If online, fetch and cache once
    if (typeof navigator !== 'undefined' && navigator.onLine) {
      console.log(`🌐 [OfflineDocumentLoader] Fetching exact context: ${url}`);
      const resp = await fetch(url, { headers: { Accept: 'application/ld+json, application/json' } });
      if (!resp.ok) throw new Error(`Failed to fetch context: ${url} (${resp.status})`);
    ...
    throw new Error(`Missing JSON-LD context offline: ${url}. Pre-cache contexts before going offline.`);
  }
}